    ...
    with tf.Session(config=config) as session:
        ...
        for i_epoch in xrange(n_epochs):
            ...
            if validation_loss is not None:
                # log += ", validation loss: {:.5f}".format(validation_loss)
                log += ", validation loss: " + str(validation_loss)
            if save_best_val_model_fn is not None and cur_validation_loss < best_validation_loss:
